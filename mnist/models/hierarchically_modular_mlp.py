...
class hierarchically_modular_mlp(nn.Module):
    def __init__(self, num_img_modules = [2], num_modules=[3,2], img_module_arch=[784,256,256,3], module_arch=[2,12,1], input_dim=2, output_dim=2, topk=2, use_gumbel=False, per_sample_routing=False, tau=1.0, known_routing=False, module_inp_indices=None):
        super(hierarchically_modular_mlp, self).__init__()
        ...
        for i in range(len(num_img_modules)):
            ...
            for j in range(num_img_modules[i]):
                self.img_modules[i].append(module_mlp(img_module_arch))
