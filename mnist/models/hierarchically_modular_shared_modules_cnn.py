...
class NNmodules_img(nn.Module):
    def __init__(self, num_modules=1, module_arch=[256,3]):
        ...
        for i in range(num_modules):
            self.cnns.append(module_cnn(module_arch))
    
    def forward(self, x, score):
        for i in range(self.num_modules):
            module_out = self.cnns[i](x)
            if i==0:
...
class hierarchically_modular_shared_modules_cnn(nn.Module):
    def __init__(self, num_img_modules=1, num_modules=2, num_img_slots=[2], num_slots=[3,2], img_module_arch=[256,3], module_arch=[2,12,1], input_dim=2, output_dim=2, topk=2, use_gumbel=False, per_sample_routing=False, tau=1.0, known_routing=False, known_module_location=False, module_inp_indices=None, module_slot_indices=None):
        super(hierarchically_modular_shared_modules_cnn, self).__init__()
