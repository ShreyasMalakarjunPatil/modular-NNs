...
def gumbel_sigmoid(logits, k: int = 2, hard: bool = False, dim: int = 1, use_gumbel=True, tau: float = 1.0) : 
    ...
    if hard: 
        ...
        ret = []  ## Newly added from here to do any k in topk
        for k1 in range(k):
            y_hard = torch.zeros_like(logits, memory_format=torch.legacy_contiguous_format)
            y_hard.scatter_(1, indices[:,k1].view(indices[:,k1].size(0),1, indices[:,k1].size(1)), 1)
            ret1 = y_hard - y_soft.detach() + y_soft
            ret.append(ret1)
    #else:
    #    ret1 = y_soft
    #    ret2 = y_soft
