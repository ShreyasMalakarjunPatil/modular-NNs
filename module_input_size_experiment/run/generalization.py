    ...
    if os.path.exists(path) and os.path.getsize(path) > 0:
        ...
        if args.model == 'mlp':
            ...
            if args.known_module_inputs:
                ...
                module_inp_indices = load.load_routing(args)
                model = load.load_model(args.model)(args.num_modules, args.module_arch, args.num_tasks, args.modularity[0], args.modularity[-1], args.topk, args.use_gumbel, args.per_sample_routing, args.tau, args.known_module_inputs, module_inp_indices, args.known_func_outputs).to(dev)
                optimizer = opt([
