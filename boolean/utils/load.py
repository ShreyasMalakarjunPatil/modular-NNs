...
def load_data(dataset_path, modularity, batch_size, dataset_noise=False, dataset_split=None, dataset_split_seed=None):

    path = dataset_path + str(modularity) + '/boolean' + str(modularity) + '.pkl'
    ...
    if dataset_noise:
        dataset = CustomTensorDataset(tensors=(X, Y), task_id=0, transform=AddGaussianNoise, std=0.1)
    else:
        dataset = CustomTensorDataset(tensors=(X, Y), task_id=0)
...
def load_routing(args):
    ...
    if (args.model == 'hierarchically_modular' and args.known_module_inputs) or (args.model == 'hierarchically_modular' and args.known_func_outputs):
        ...
        for i in range(args.num_tasks):
            dataset_name = args.dataset_path + str(args.modularity) + '/boolean' + str(args.modularity) + '_module_input_indices.pkl'
    ...
    if (args.model == 'hierarchically_modular_shared_modules' and args.known_module_inputs and args.known_module_locations) or (args.model == 'hierarchically_modular_shared_modules' and args.known_func_outputs):
        ...
        for i in range(args.num_tasks):
            ...
            dataset_name = args.dataset_path + str(args.modularity) + '/boolean' + str(args.modularity) + '_module_input_indices_shared_modules.pkl'
            with open(dataset_name, "rb") as fout:
                a = pkl.load(fout)
            module_input_indices.append(a)

            dataset_name = args.dataset_path + str(args.modularity) + '/boolean' + str(args.modularity) + '_slot_module_indices_shared_modules.pkl'
            with open(dataset_name, "rb") as fout:
                a = pkl.load(fout)
